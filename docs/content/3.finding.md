...
description: Learn how to search, filter, and discover color schemes programmatically.
---

# Finding Schemes in ColorSchemes

`color-schemes` provides the function `getInverse(cscheme, color)`, which is the *inverse* of `get(cscheme, t)`. Given an RGB color value, this function evaluates the scheme and returns a normalized value between `0.0` and `1.0` corresponding to the closest matching position along the scheme's axis.

```typescript
import { getInverse, colorschemes } from 'color-schemes';
